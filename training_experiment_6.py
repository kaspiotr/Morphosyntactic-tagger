...
from training import map_paragraph_id_to_text_category_name, _write_paragraph_to_file, use_scratch_dir_if_available, \
    prepare_skf_splits_data_folder, prepare_output_file_path
...
def train_sequence_labeling_model(data_folder, proposed_tags_vocabulary_size, skf_split_no):
    ...
    Model and training logs are saved in resources_ex_6/taggers/example-pos/it-<skf_split_no> directory (where
    ...
    Additionally method logs other training log files and saves them in the resources_ex_6 directory of this project
    under the name training_ex_6_<skf_plit_no>.log
    ...
    # 7. start training
    trainer.train(use_scratch_dir_if_available('resources_ex_6/taggers/example-pos/it-' + str(skf_split_no)),
                  learning_rate=0.1,
                  mini_batch_size=32,
    ...
    plotter = Plotter()
    plotter.plot_weights(use_scratch_dir_if_available('resources_ex_6/taggers/example-pos/it-' + str(skf_split_no)
                                                      + '/weights.txt'))
...
def train(skf_split_no, jsonl_file_path):
    ...
    - WordEmbeddings - classic word embeddings. That kind of embeddings are static and word-level, meaning that each
      ...
      used.
    Model training is based on stratified 10 fold cross validation split indicated by skf_split_no argument.
    Model and training logs are saved in resources_ex_6/taggers/example-pos directory/it-<skf_split_no> (where
    <skf_split_no> is the number of stratified 10 fold cross validation split used to train the model).
    Additionally method logs other training log files and saves them in the resoruces_ex_6 directory of this project
    under the name training_ex_6_<skf_plit_no>.log
    ...
    that file
    """
    log.basicConfig(filename=use_scratch_dir_if_available('resources_ex_6/training_ex_6_' + str(skf_split_no) + '.log'),
    ...
    data_folder = prepare_skf_splits_data_folder('data_ex_6')
