 ...
 * First experiment:  
   ...
   * **convert_nkjp_to_xml_ex_1.py**:  
   This script generates *nkjp_ref_n* files for each stratified 10-fold cross validation split (where *n* is the number of split) corresponding to given *test_n.xml* file generated by **convert_tsv_to_xml_ex_1.py** script. Files *nkjp_ref_n.xml* are used by [*tagger-eval.py*](https://github.com/kaspiotr/Morphosyntactic-tagger-evaluation/blob/master/tagger-eval.py) script to evaluate tagger.
   Files *nkjp_ref_n.xml* are saved in _resources_ex_1_ directory of this project. To run this script you need to create a _nkjp_data_ex_1_ directory in the main directory of this project.  
   This script could also be ran on _Prometheus_ with use of following script (it is essential to run it on the same host training was done due to get the same SKF split to this made during training of the model):
     - [convert_nkjp_to_xml_ex_1.sh](https://github.com/kaspiotr/Morphosyntactic-tagger/blob/master/convert_nkjp_to_xml_ex_1.sh)
 * Second experiment:  
