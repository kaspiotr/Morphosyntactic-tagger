...
def train_sequence_labeling_model(data_folder, proposed_tags_vocabulary_size, skf_split_no):
    ...
    # 7. start training
    trainer.train('resources_ex_4/taggers/example-pos/it-' + str(skf_split_no),
                  learning_rate=0.1,
                  mini_batch_size=32,
    ...
    plotter = Plotter()
    plotter.plot_weights('resources_ex_4/taggers/example-pos/it-' + str(skf_split_no) + '/weights.txt')
...
def train(skf_split_no, jsonl_file_path):
    ...
    that file
    """
    log.basicConfig(filename='resources_ex_4/training_ex_4_' + str(skf_split_no) + '.log',
    ...
    data_folder = os.path.dirname(os.path.abspath(__file__)) + '/data_ex_4'
