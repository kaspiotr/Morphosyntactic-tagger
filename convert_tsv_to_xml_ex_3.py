...
def convert_tsv_to_xml(tsv_file_path):
    ...
    root = tree.getroot()
    with open("resources_ex_3/test_" + split_no + ".xml", "w", encoding='UTF-8') as xf:
...
def read_sentence_from_tsv(skf_split_no):
    with open("resources_ex_3/taggers/example-pos/it-" + skf_split_no + "/test.tsv") as fd:
...
def main():
    path = os.path.abspath(os.path.dirname(os.path.abspath(__file__))) + "/resources_ex_3/taggers/example-pos/*/test.tsv"
